...
export class MockDataService {
    ...
    // ==========================================================
    totalXp = signal(380);
    completedChallengeIds = signal(new Set());
    awardedGoalIds = signal(new Set());
    xpLevels = XP_LEVELS;
    currentLevel = computed(() => this.xpLevelFor(this.totalXp()));
    nextLevel = computed(() => {
    ...
    unlockBadge(id) {
        this.badges.update((list) => list.map((b) => (b.id === id ? { ...b, earned: true } : b)));
    }
    ...
    completeChallenge(id) {
        if (this.completedChallengeIds().has(id))
            return;
        const challenge = this.challenges().find((c) => c.id === id);
        if (!challenge)
            return;
        this.completedChallengeIds.update((set) => new Set(set).add(id));
        this.challenges.update((list) => list.map((c) => (c.id === id ? { ...c, progress: 100 } : c)));
        this.totalXp.update((xp) => xp + challenge.xp);
        if (challenge.badgeId)
            this.unlockBadge(challenge.badgeId);
        if (this.completedChallengeIds().size >= 5)
            this.unlockBadge('b3');
    ...
    recordGoalCompletion(goal) {
        if (goal.id == null || goal.status !== 'Achieved')
            return;
    ...
    toggleChallenge(id) {
        this.challenges.update((list) => list.map((c) => (c.id === id ? { ...c, joined: !c.joined } : c)));
    }
    updateGoalProgress(id, current) {
        this.goals.update((list) => list.map((g) => (g.id === id ? { ...g, current } : g)));
    }
    static ɵfac = function MockDataService_Factory(t) { return new (t || MockDataService)(); };
    static ɵprov = /*@__PURE__*/ i0.ɵɵdefineInjectable({ token: MockDataService, factory: MockDataService.ɵfac, providedIn: 'root' });
}
(() => { (typeof ngDevMode === "undefined" || ngDevMode) && i0.ɵsetClassMetadata(MockDataService, [{
        type: Injectable,
